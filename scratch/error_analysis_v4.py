...
def eng_bin(df):
    ...
    df['f_disclosure_hits'] = t.str.lower().apply(lambda x: sum(1 for p in ['confidential', 'merger', 'acquisition'] if p in x))
...
def analyze_binary():
    ...
    print("="*40)
    data_path = os.path.join(ROOT, 'binary_pipeline', 'data', 'emails_labeled_silver_tenK.parquet')
...
def analyze_multiclass():
    ...
    data_path = os.path.join(ROOT, 'multiclass_pipeline', 'data', 'emails_labeled_silver_tenK.parquet')
    df = pd.read_parquet(data_path)
    # Correct multiclass preprocessing combining subject and body
    ...
    for m in ['lr', 'rf', 'xgb']:
        ...
        if os.path.exists(path):
            ...
            try:
                ...
                print(f"{m.upper():<5} | Prediction Error: {e}")
